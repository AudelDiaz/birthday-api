...
def read_root(request: Request):
    return {
        "Developed by": "https://github.com/AudelDiaz/",
...
def get_birthdays_by_month(month: int, db: Session = Depends(get_db)):
    if 1 <= month <= 12:
        ...
        raise HTTPException(
            status_code=400, detail="The month param should be a number between 1 and 12.")
...
def get_birthday_by_day(month: int, day: int, db: Session = Depends(get_db)):
    if 1 <= month <= 12 and 1 <= day <= 31:
        ...
        raise HTTPException(
            status_code=400, detail="The day param should be a number between 1 and 31.")
    else:
        raise HTTPException(
            status_code=400, detail="The month param should be a number between 1 and 12.")
